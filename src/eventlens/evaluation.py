...
def group_holdout_split(
    ...
    groups = sorted({getattr(article, group_field) or "UNKNOWN" for article in articles})
    ...
    train = [article for article in articles if (getattr(article, group_field) or "UNKNOWN") not in holdout_groups]
    valid = [article for article in articles if (getattr(article, group_field) or "UNKNOWN") in holdout_groups]
    return train, valid
...
def evaluate_split(
    ...
    model = train_baseline(train_labeled)
...
def render_generalization_report(articles: list[ArticleRecord]) -> str:
    ...
    splitters: list[tuple[str, Callable[[list[ArticleRecord]], tuple[list[ArticleRecord], list[ArticleRecord]]]]] = [
        ("时间切分", time_split),
        ("公司切分", lambda rows: group_holdout_split(rows, "entity")),
